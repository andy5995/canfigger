...
main (void)
{
  ...
  const char *data1[] = {"one" , "two", "three", "forty-four","fifty five", "six", "7", "eight" };
  const char *data2[] = { "Mercury", "Venus", "Earth", "Mars", "Jupiter", "Saturn", "Neptune", "Uranus", "Pluto" };
  const char *data3[] = {
    "second",
    "after one",
  };
...
Attribute: %s\n", list->key, list->value, list->attr_node_next->str);
    ...
    int j = 0;
    while (list->attr_node_next != NULL)
    {
      fprintf (stderr, "attr: %s\n", list->attr_node_next->str);
      switch (i)
      {
        case 0:
            assert (strcmp (data1[j], list->attr_node_next->str) == 0);
            break;
        case 1:
            assert (strcmp (data2[j], list->attr_node_next->str) == 0);
            break;
        case 2:
            assert (strcmp (data3[j], list->attr_node_next->str) == 0);
            break;
        default:
          break;
      }
      j++;
      list->attr_node_next = list->attr_node_next->next;
    }
